...
def Total_Heating(array,t,w,l,h,RT,ST,I_Max,Rup,Rdwn,Ht,RP,n):
    ...
    if ST < RT:
        ...
        i=1
        while Total_Temp[i-1,3] < RT:
            dT1 = Rate_of_heating(t,w,l,h,Total_Temp[i-1,1],RP)
            Total_Temp[i,2] = dT1
...
def submit1(text):
    tdata = eval(text)    
    Total_Temp = Total_Heating(earray,tdata,w,l,lb,h,RT,ST,I_Max,Rup,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()

def submit2(text):
    wdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,wdata,l,lb,h,RT,ST,I_Max,Rup,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit3(text):
    ldata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,ldata,lb,h,RT,ST,I_Max,Rup,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit4(text):
    lbdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lbdata,h,RT,ST,I_Max,Rup,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit5(text):
    hdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,hdata,RT,ST,I_Max,Rup,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit6(text):
    RTdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,h,RTdata,ST,I_Max,Rup,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit7(text):
    STdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,h,RT,STdata,I_Max,Rup,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit8(text):
    I_Maxdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,h,RT,ST,I_Maxdata,Rup,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit9(text):
    Rupdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,h,RT,ST,I_Max,Rupdata,Rdwn,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit10(text):
    Rdwndata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,h,RT,ST,I_Max,Rup,Rdwndata,Ht,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit11(text):
    Htdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,h,RT,ST,I_Max,Rup,Rdwn,Htdata,P,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit12(text):
    Pdata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,h,RT,ST,I_Max,Rup,Rdwn,Ht,Pdata,n)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
def submit13(text):
    ndata = eval(text)    
    Total_Temp = Total_Heating(earray,t,w,l,lb,h,RT,ST,I_Max,Rup,Rdwn,Ht,P,ndata)
    TTemp.set_ydata(Total_Temp[:,3])
    I_in.set_ydata(Total_Temp[:,1])
    #ax.set_ylim(np.min(ydata), np.max(ydata))
    plt.draw()
...
#Temperatures
RT_box = plt.axes([0.55, 0.6, 0.1, 0.05])
ST_box = plt.axes([0.7, 0.6, 0.1, 0.05])

#Ramp Profile
I_Max_box = plt.axes([0.55, 0.5, 0.1, 0.05])
Rup_box = plt.axes([0.7, 0.5, 0.1, 0.05])
Rdwn_box = plt.axes([0.85, 0.5, 0.1, 0.05])
Ht_box = plt.axes([0.55, 0.42, 0.1, 0.05])


#Pressure & Bolts
P_box = plt.axes([0.55, 0.32, 0.1, 0.05])
#B_box = plt.axes([0.1, 0.05, 0.8, 0.075])
#Ab_box = plt.axes([0.1, 0.05, 0.8, 0.075])
...
text_box7.on_submit(submit7)
text_box8.on_submit(submit8)
text_box9.on_submit(submit9)
text_box10.on_submit(submit10)
text_box11.on_submit(submit11)
text_box13.on_submit(submit13)
