...
def get_average_of_tap(files_name,data,Current_indices,tap_of_interest):
    ...
    for j in range(0,len(files_name)):
        li1.append(data.iloc[:,5*j+(tap_of_interest)].iloc[int(Current_indices[j,0]):int(Current_indices[j,1])])
...
def noise_range(files_name,data,Istart,Iend):
    ...
    for i in range(0,len(files_name)):
        I_nAp = np.where(data.iloc[:,5*i] > Istart)
        I_nA.append(I_nAp[0])   
        ...
        if Iend >= max(InVs_1.iloc[:,5*i]):  
            ...
            I_nBp = np.where(data.iloc[:,5*i] > Iend)
            I_nB.append(I_nBp[0])    
...
def find_noise_per_tap(Noice_indices,data,filt_before):
    ...
    if filt_before == 1:    
        for j in range(0,len(all_files)):
            for i in range(0,4):
                Avg_noise1.append(np.mean(butter_lowpass_filter(cutoff, fs, order,data.iloc[int(Noice_indices[j,0]):int(Noice_indices[j,1]),j*5+i+1])))   
    else:
        for j in range(0,len(all_files)):
            for i in range(0,4):
                Avg_noise1.append(np.mean(data.iloc[int(Noice_indices[j,0]):int(Noice_indices[j,1]),j*5+i+1]))
...
InVs_1 = Extract_Voltages(all_files,4,"CmdAmps.Value","VTapFilter","rVal")
...
Noice_ind = noise_range(all_files,InVs_1,200, 1000)
...
first_guess = [1, Ic_guess, VFloor_guess ,n_guess]
#fit function
popt, pcov = curve_fit(func, Avg_InV_tap.iloc[:,0], Avg_InV_tap.iloc[:,1]-n_p_t_avg[int(tap_of_interest)-1], 
                       p0=first_guess,sigma=Avg_InV_tap.iloc[:,0]/20, absolute_sigma=True)
...
popt, pcov = curve_fit(func, Avg_InV_tap.iloc[:,0], Avg_InV_tap.iloc[:,1]-n_p_t_avg[int(tap_of_interest)-1], 
                       p0=first_guess,sigma=Avg_InV_tap.iloc[:,0]/20, absolute_sigma=True)
#popt, pcov = curve_fit(func, Avg_InV_tap.iloc[:,0], butter_lowpass_filter(cutoff, fs, order,Avg_InV_tap.iloc[:,1]), p0=first_guess)
#popt, pcov = curve_fit(func, Avg_InV_tap.iloc[:,0], butter_lowpass_filter(cutoff, fs, order,Avg_InV_tap.iloc[:,1])-n_p_t_avg[int(tap_of_interest)-1], p0=first_guess)
...
#%%
plt.plot(InVs_1.iloc[:,0],InVs_1.iloc[:,1])
plt.show()